...
class GUI:
    def __init__(self,other):
        # self is object of class GUI, other is object of class Tinder such that object of class Tinder can access all the functions like other.loginHandler()/other.regHandler
        ...
        # GUI uses the funtion other.loginHandler() to fetch email and password from GUI box and sends it to the loginHandler in tinder which is then searched in database
        ...
        self._root.mainloop()   #to hold the screen

    def openpic(self):
        #self._root1.filename = filedialog.askopenfilename(initialdir="D:\Pics")
        #imageUrl = self._root1.filename
        #global my_image
        #path = "image/image3.jpg"
        #my_image = ImageTk.PhotoImage(Image.open(path))
        #my_image_label = Label(master=self._root1, image=my_image).grid(row=1,column=1)
        pass

    def regWindow(self,other,destroy=0):         #destroy=0: opens the registration page
        if(destroy==0):                          #destroy=1: destroys the registraion page
            self._root1=Tk()
            ...
            self._picBTN=Button(self._root1,text="Select Profile Pic",command=lambda :self.openpic())
            self._picBTN.grid(row=1,column=0)
            ...
            self._registerBtn = Button(self._root1, text="Register",bg="#f4d03f", fg='#c0392b',width=10, height=2,command=lambda:other.checker(self._nameInput.get(),self._emailInput.get(),self._passwordInput.get(),self._ageInput.get(),clicked.get(),self._cityInput.get(),self._bioInput.get()))
            self._registerBtn.grid(row=9,column=1,pady=(10,10),padx=(10,10))                                            #checker checks whether the given email is already registered or not
    ...
    def mainWindow(self,other,user_id,data_own,mode,num=0):             #data is the information of logged in user which is being provided from the doLogin()
        self.clearScreenr()                                             #mainWindow() is being reused as main window for showing own profile/for viewing others profile/for deleting account/for editing
        self.clearScreeng()
        self.headermenu(other,user_id,data_own)

        data = other._dbObject.get_details(user_id)                      #mode=0 shows the details of users(whether the current user or other users)
                                                                         #mode=1 shows details of logged in user through out the application(details of current user can be accessed anytime)
        if(mode==0):                                                     #mode=2 shows the information of other users where you can move previous,propose,next
            imageUrl = "image/image1.jpg"
            load = Image.open(imageUrl)
            load = load.resize((200, 200), Image.ANTIALIAS)             #mode=3 shows the details of logged in users and DELETE button to delete the account
                                                                        #mode=4 is the edit window
            render = ImageTk.PhotoImage(load)                           #mode=5 to logout the user
                                                                        #mode=6 to destroy the window
            img = Label(image=render)                                   #mode=7 to see the RECEIVED PROPOSAL list
            img.image = render                                          #mode=8 to see the SENT PROPOSAL list
            img.grid(row=0, column=2, rowspan=2, columnspan=2)          #mode=9 to see the MATCHES list
        ...
        if(mode==4):
            ...
            self._deleteBtn.grid(row=0, column=0,pady=(10,10),padx=(10,10))                           #redirects you to the page where thes users data is displayed and DELETE button is used to delete the account

        if mode==5:
            self.mainWindow(other,user_id,data_own,mode=6)     #destroys the MAINWINDOW
            self.__init__()                        #displays the main login page

        if mode==6:
            self._root.destroy()     #destroys the MAINWINDOW
    ...
    def printMessage(self, title, message,mode):
        master = Tk()                                                       #mode=0: show error message
        master.withdraw()                                                   #mode=1: ask qestion(YES or NO)
        if(mode==0):                                                        #mode=2: show info
